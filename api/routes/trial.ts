...
// POST /api/trial/register - Register new trial user
router.post('/register', async (req, res) => {
  try {
    ...
    // Calculate trial expiration (14 days from now, but only starts after email verification)
    const now = new Date()
    ...
    const { data: newUser, error: userError } = await supabase
      ...
      .insert({
        ...
        is_trial_user: true,
        trial_started_at: null, // Will be set when email is verified
        trial_expires_at: null, // Will be set when email is verified
...
// POST /api/trial/verify-email - Verify email address
router.post('/verify-email', async (req, res) => {
  try {
    ...
    const now = new Date()
    const trialExpires = new Date(now.getTime() + (14 * 24 * 60 * 60 * 1000)) // 14 days
...
// POST /api/trial/resend-verification - Resend verification email
...
// GET /api/trial/check-expired - Check and suspend expired trial users
router.get('/check-expired', async (req, res) => {
  try {
    console.log('Checking for expired trial users')
    
    const now = new Date()
    
    // Find expired trial users
    const { data: expiredUsers, error: findError } = await supabase
      .from('users')
    ...
    if (!expiredUsers || expiredUsers.length === 0) {
      return res.json({ message: 'No expired trial users found', count: 0 })
    }
    ...
    if (suspendError) {
      ...
      return res.status(500).json({ error: 'Failed to suspend expired users' })
    }
    
    console.log(`Successfully suspended ${expiredUsers.length} expired trial users`)
    
    res.json({ 
      message: `Successfully suspended ${expiredUsers.length} expired trial users`,
      count: expiredUsers.length,
      users: expiredUsers.map(user => ({ id: user.id, email: user.email, name: user.name }))
    })
